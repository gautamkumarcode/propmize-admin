...
import { NotificationService } from "@/lib/services/notificationServices";
...
export const useNotifications = () => {
	...
	const handleNotificationClick = useCallback(
		async (notification: NotificationTypes) => {
			try {
				if (!notification.read) {
					await markAsRead(notification._id);
				}
				const actionUrl =
					notification.metadata?.actionUrl || notification.actionUrl;
	...
	const addNotification = useCallback(
		(
			notification: Omit<NotificationTypes, "_id" | "createdAt" | "updatedAt">
		) => {
